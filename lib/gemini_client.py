...
class GeminiClient:
    ...
    def _build_prompt(self, prompt: str, context: list = None, mode: str = "general") -> str:
        ...
        if is_web_context:
            # Веб-источники (Tavily)
            sources_list = "\n".join([
                f"[{i}] {c.get('title', 'No title')} — {c.get('url', '')}"
                for i, c in enumerate(context[:3], 1)
            ])

            full_prompt = f"""{system_prompt}

📚 Источники из интернета (Tavily AI):
{sources_list}
...
2. В тексте ответа используй ссылки на источники в формате [1], [2], [3]
